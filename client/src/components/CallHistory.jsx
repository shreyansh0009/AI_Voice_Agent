...
export default function CallHistory() {
  ...
  // Start listening to a call via WebSocket
  const startListening = useCallback((callId) => {
    // Close any existing spy connection
    if (spyWsRef.current) {
      spyWsRef.current.close();
      spyWsRef.current = null;
    }
    ...
    ws.onmessage = (event) => {
      ...
      // First byte is source marker (0x01=caller, 0x02=agent), rest is slin16 PCM
